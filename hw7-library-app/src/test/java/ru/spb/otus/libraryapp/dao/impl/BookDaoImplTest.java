...
class BookDaoImplTest {
    ...
    @Test
    void findById() {
        Assertions.assertNotNull(bookDao.findById(BASE_BOOK_ID).orElse(null));
    }
    ...
    void update() {
        ...
        bookDao.update(newName, newDescription, BASE_BOOK_ID);

        Book book = bookDao.findById(BASE_BOOK_ID).orElse(null);
    ...
    void deleteById() {
        ...
        Long count = bookDao.findAll()
                .stream()
                .filter(book -> book.getId() == DELETE_BOOK_ID)
                .count();
