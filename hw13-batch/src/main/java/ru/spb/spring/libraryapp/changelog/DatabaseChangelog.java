...
public class DatabaseChangelog {
    ...
    public void someChange2(MongoTemplate mongoTemplate) {
        ...
        Author author = Author.builder().firstName("first name2").lastName("last name2").patronymic("patronymic").build();
    ...
    @ChangeSet(order = "005", id = "genre", author = "avetall87")
    public void someChange5(MongoTemplate mongoTemplate) {
        Genre genre = Genre.builder().name("new test genre").build();
        mongoTemplate.save(genre);

        Book book = Book.builder().name("book_genre").build();
        book.setGenres(List.of(genre));
        mongoTemplate.save(book);

    }

    @ChangeSet(order = "006", id = "addBookWithAuthor2", author = "avetall87")
    public void someChange6(MongoTemplate mongoTemplate) {
    ...
    @ChangeSet(order = "007", id = "addBookWithAuthor3", author = "avetall87")
    public void someChange7(MongoTemplate mongoTemplate) {

        Author author = Author.builder().firstName("first_name2").lastName("last_name2").patronymic("patronymic2").build();
        mongoTemplate.save(author);
